...
// Modify the addTrailingZero function to always return a two-digit string
function addTrailingZero(number) {
  return number.toString().padStart(2, '0');
}

// Modify the updateTime function to use addTrailingZero for hours, minutes, and seconds
const updateTime = () => {
  const time = new Date();
  let hours = addTrailingZero(time.getHours());
  let minutes = addTrailingZero(time.getMinutes());
  let seconds = addTrailingZero(time.getSeconds());
